...
SET OUTPUT=%APPVEYOR_BUILD_FOLDER%/windows_%BUILD_VERSION%.zip    
SET PARAM=--header "Content-Type: application/zip" --upload-file @%OUTPUT% https://uploads.github.com/repos/waliarubal/Jaya/releases/v%BUILD_VERSION%/assets?name=windows.zip
START /wait /B "" %CURL% %PARAM%

ECHO;
ECHO Upload Linux binaries
SET OUTPUT=%APPVEYOR_BUILD_FOLDER%/linux_%BUILD_VERSION%.zip    
SET PARAM=--header "Content-Type: application/zip" --upload-file @%OUTPUT% https://uploads.github.com/repos/waliarubal/Jaya/releases/v%BUILD_VERSION%/assets?name=linux.zip
START /wait /B "" %CURL% %PARAM%

ECHO;
ECHO Upload Mac OS binaries
SET OUTPUT=%APPVEYOR_BUILD_FOLDER%/osx_%BUILD_VERSION%.zip    
SET PARAM=--header "Content-Type: application/zip" --upload-file @%OUTPUT% https://uploads.github.com/repos/waliarubal/Jaya/releases/v%BUILD_VERSION%/assets?name=osx.zip
START /wait /B "" %CURL% %PARAM%